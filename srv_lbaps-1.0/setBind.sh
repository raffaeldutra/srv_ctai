# Criado em: 2011/05/07 (Sáb) 13:53:57 (BRT)
...
cat << EOT >> $bindPrefix/named.conf.local
zone "$(echo $(echo -n "$(checkIps)." | tac -s. | cut -d . -f2-4).in-addr.arpa)"
{
    type master;
...
# aqui sao setado os subdominios do dominio, caso seja necessario mais, apenas acrescentar abaixo
    for netServices in @ ns www ftp mail ntp
    do
cat << EOT >> $bindPrefix/zones/$doCreateZone.$nameDomain
$netServices    IN  A   $(checkIps)
EOT
    done
done
...
;
@	IN	NS	ns.$doCreateZone.$nameDomain.
$(echo $(echo -n "$(checkIps)." | tac -s. | cut -d . -f1)) IN  PTR $doCreateZone.$nameDomain.
EOT
